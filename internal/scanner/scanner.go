...
func Run(ctx context.Context, cfg Config) (Result, error) {
	...
	mcpEmitFinding := func(f model.Finding) {
		if err := cfg.Emitter.EmitFinding(f); err != nil {
			setEmitErr(fmt.Errorf("emit credential finding: %w", err))
			return
		}
		findingsMu.Lock()
		findingsEmitted++
	...
	mcpS := &mcp.Scanner{MaxFileSize: cfg.MaxFileSize, Emit: emit, EmitFinding: mcpEmitFinding, Diag: diag}
